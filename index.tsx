...
const MetaSnippet = ({ snippet, onUpdate, onDelete }: SnippetProps<'meta'>) => (
    <SnippetCard title="Meta Data" snippetId={snippet.id} onDelete={onDelete}>
        <div className="grid gap-4">
            ...
            <div className="grid gap-2">
                <Label htmlFor={`audio-${snippet.id}`}>Audio URL</Label>
                <Input id={`audio-${snippet.id}`} placeholder="https://example.com/..." value={snippet.data.audioUrl} onChange={e => onUpdate(snippet.id, { ...snippet.data, audioUrl: e.target.value })} />
            </div>
        </div>
    </SnippetCard>
);
